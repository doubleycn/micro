...
func try(blockName string, t *t, f cmdFunc, maxTime time.Duration) {
	...
	for {
		...
		if time.Since(start) > maxTime {
			...
			if err != nil {
				t.Fatalf("%v:%v, %v (failed after %v with '%v'), output: '%v', config: '%+v', config errs: %+v", fname, line, blockName, time.Since(start), err, string(outp), string(config.All()), config.Errors())
			}
			return
		}
		outp, err = f()
